...
//        case Qt::Key_P:
//            gameTimer->stop();
//            QMessageBox::information(this, "Paused", "Game Paused. Press OK to resume.");
//            gameTimer->start(snakeLogic.getSpeed());
//            break;
//        case Qt::Key_Q:
//            close();
...
//    if (snakeSegments.empty()) return;  // Check if the snake vector is empty to avoid further access

//    for (const auto& segment : snakeSegments) {
//        if (segment.x < 0 || segment.y < 0 || segment.x >= height() || segment.y >= width()) continue;  // Skip invalid segments (ensure in bounds)
//        painter.drawRect(segment.y * 15, segment.x * 15, 15, 15);  // Draw rect at segment position
//    }
//}
...

{
  ...
  connect(startButton, &QPushButton::clicked, this, &SnakeGameWidget::startGame);
...
void SnakeGameWidget::paintEvent(QPaintEvent *event)
{
  ...
  for (int i = 1; i < snakeGame->getWidth() + 1; ++i) {
    for (int j = 1; j < snakeGame->getHeight() + 1; ++j) {
      if ((i + j) % 2 == 0) {
        painter.fillRect(i * cellSize, + j * cellSize, cellSize, cellSize, turquoiseColor);
      } else {
  ...
  for (const auto &block : snakeGame->getSnake()) {
    painter.drawEllipse(block.y * cellSize, block.x * cellSize, cellSize, cellSize);
  }

  s21::SnakeLogic::Point fruitPosition = snakeGame->getFood();
  painter.setBrush(Qt::red);
  painter.drawEllipse(fruitPosition.y * cellSize, fruitPosition.x * cellSize, cellSize, cellSize);
...
void SnakeGameWidget::updateGame()
{
  if (!snakeGame->getSnake().empty() && !snakeGame->getGameOver()) {
    ...
    if (snakeGame->getGameOver()) {
      ...
      gameOverMsgBox.setText("Game Over! Your Score: " + QString::number(snakeGame->getCurrentScore()));
...
void SnakeGameWidget::stopGame()
{
  snakeTimerRunning = false;
}

void SnakeGameWidget::updateInfoWidget()
{

  infoWidget->setText("Level: " + QString::number(snakeGame->getCurrentLevel()) + "\n"
